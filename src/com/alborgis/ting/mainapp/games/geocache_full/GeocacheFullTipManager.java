...
public class GeocacheFullTipManager {
	...
	public static void showTipsForMapStartup(Context ctx, SharedPreferences prefs, boolean forceShow, boolean modoDemo, boolean geolocatedGame){
		...
		if(modoDemo){
			...
			tip1.message = "¡Bienvenido al juego del Geocaching Full! Estás jugando en modo DEMO. Recuerda que puedes jugar con tu cuenta de Travel in Games o de Facebook para poder obtener logros, ganar premios y guardar la partida";
			tip1.gravity = Gravity.BOTTOM;
			wizard.addTip(tip1);
			
			TINGTip tip2 = new TINGTip();
			tip2.title = "OBJETIVO";
			tip2.message = "Tu objetivo es encontrar todos los tesoros que Gully ha ocultado en los cofres que ves en el mapa";
			tip2.gravity = Gravity.BOTTOM;
			wizard.addTip(tip2);
		}else{
			TINGTip tip1 = new TINGTip();
			tip1.title = "¡BIENVENIDO AL JUEGO!";
			tip1.message = "¡Bienvenido al juego del Geocaching Full! Tu objetivo es encontrar todos los tesoros que Gully ha ido dejando en los cofres que ves en el mapa";
			tip1.gravity = Gravity.BOTTOM;
			wizard.addTip(tip1);
		}
		...
		if(geolocatedGame){
			...
			tip1.message = "Este juego es 'Indoor'. Quiere decir que puedes estar en cualquier lugar para abrir los cofres y encontrar los tesoros";
			tip1.gravity = Gravity.BOTTOM;
			wizard.addTip(tip1);
		}
